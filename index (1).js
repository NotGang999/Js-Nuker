...
client.on("ready", () => {
    ...
    console.log(red('           ════════════════════════════════════════════════════════════════════════════════'));
    console.log("");
    client.user.setActivity({ type: "PLAYING", name: "CrimeX Op" }); // Delete this if you wish.
});
...
\` [text]
            ...
            **Server Annihilation:** \`${prefix}die\` [text]
            `)
            .setThumbnail(``)
            .setFooter(`© CrimeX Nuker | Prefix: ${prefix} Made By: ${founder}`)
            .setColor(0x36393E)
    ...
    if (disableEveryone === false) {
        ...
        if (message.content.startsWith(prefix + 'help')) {
            const helpEmbed = new Discord.MessageEmbed()
                ...
                **Server Annihilation:** \`${prefix}die\` [text]
                `)
                .setImage(`https://cdn.discordapp.com/attachments/887988384335818782/891209413610852372/standard_1.gif`)
                .setFooter(`© Presser | Prefix: ${prefix} | Author: DHRUV GOEL | Cord: ${founder}`)
                .setColor(0x36393E)
                .setTimestamp(Date.now());
            message.channel.send(helpEmbed);
            message.delete();
